...
solutionWithStep ::
     (Accnf -> Float -> State -> State) -> Accnf -> Float -> State -> [State]
...
displayStates = foldr f ""
  where
    f (t, p, v) str =
      str ++
      "T:" ++
      (printf "%.4f" t :: String) ++ " P:" ++ show p ++ " V:" ++ show v ++ "\n"

-- Projectile fire at angle theta with velocity v has x, y componenst v cos and v sin
projectileInit :: Float -> Float -> V.Vector
projectileInit v theta =
  V (v * cos (theta * 0.0174533), v * sin (theta * 0.0174533), 0)
...
projectileAtVandTheta v theta =
  takeWhile heightPositive $
  solutionWithStep ecStep projectile 0.01 (0, p0, projInit)
  where
...
severalProjectiles v theta dtheta
  | theta > 0 =
    projectileAtVandTheta v theta : severalProjectiles v (theta - dtheta) dtheta
  | otherwise = []
...
plotSeveralProjectiles v theta dtheta =
  drawPics . pathPlots . paths $ severalProjectiles v theta dtheta
...
dampHosc k c (_, V (x, y, z), V (vx, vy, vz)) =
  V (-1.0 * k * x, 0, 0) ^-^ V (c * vx, vy, vz)

oneHosc :: Color -> (Accnf -> Float -> State -> State) -> Picture
oneHosc c stp =
  pathPlot c . timeVelocityXS . take 50000 $
  solutionWithStep stp (hosc 2.0) 0.01 (0, p0, v0)
  where
    p0 = V (0, 0, 0)
    v0 = V (50, 0, 0)
...
oneDampHosc c stp =
  pathPlot c . timeVelocityXS . take 50000 $
  solutionWithStep stp (dampHosc 10.0 1.0) 0.01 (0, p0, v0)
  where
    p0 = V (0, 0, 0)
    v0 = V (150, 0, 0)
...
springDamp =
  drawPics
    [ oneDampHosc red step
    , oneDampHosc blue ecStep
    , color black (line [(0, 150), (50000, 150)])
    ]
