...
polygonFromVectors vs = Polygon pts
  where
    pts = map f vs
      where
        f (V (x, y, _)) = (x, y)
...
axes :: Picture
axes =
  color red (line [(-50000, 0), (50000, 0)]) <>
